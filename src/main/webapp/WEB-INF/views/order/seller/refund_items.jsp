    ...
    <section class="main-body bg-light-subtle">
                ...
                <c:forEach items="${refundItemList}" var="item">
                    <div class="card border-secondary mb-3" style="width: 60%">
                        <div class="card-header bg-transparent border-secondary"
                            ...
                            (
                            <c:choose>
                                <c:when test="${item.refund.status == 1}">환불 중</c:when>
                                <c:when test="${item.refund.status == 2}">환불 완료</c:when>
                            </c:choose>
                            )
                        </div>
                        <div class="card-body" style="display: flex; justify-content: start">
                            <div class="text-secondary" style="width: 50%">
                                <h5 class="card-title">환불 사유 : ${item.refund.reason}</h5>
                                <p class="card-text">
                                    ...
                                    <span style="font-size: large; color: #b02a37">환불 가격 : </span>${item.totalPrice}<br>
                                </p>
                            </div>
                        </div>
        ...
        <nav aria-label="Page navigation example">
            <ul class="pagination justify-content-center">
                <li class="page-item">
                    <a class="page-link" href="/seller/order/refunds?pageNumber=${startPage - 1}"
                       aria-label="Previous">
                ...
                <c:forEach begin="${startPage}" end="${endPage}" step="1" var="page">
                    <li class="page-item"><a class="page-link ${page == currentPage ? 'active' : ''}"
                                             href="/seller/order/refunds?pageNumber=${page}">${page}</a></li>
                </c:forEach>
                <li class="page-item">
                    <a class="page-link" href="/seller/order/refunds?pageNumber=${endPage + 1}" aria-label="Next">

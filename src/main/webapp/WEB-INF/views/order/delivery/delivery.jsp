        ...
        <section class="main-content-section" style="display: flex; justify-content: center">
            <div class="card mt-3 mb-3" style="width: 80%">
                <div class="card-header text-info" style="font-size: xx-large">
                    <input type="hidden" id="delivery-id" value="${order.delivery.id}">
                    ...
                    <c:if test='${order.delivery.sender.id == auth.id}'>
                        <span>
                        <c:choose>
                            <c:when test="${order.delivery.status == 1}">
                                <button id="delivery-start-btn" class="btn btn-outline-info">배송 하기</button>
                            </c:when>
                            <c:when test="${order.delivery.status == 2}">
                ...
                <div class="card-body">
                    ...
                    <p class="card-text">
                        <span style="font-size: large;">발송 : </span>${order.delivery.sender.name}<br>
                        <span style="font-size: large;">수신 : </span>${order.userName}<br>
                        <span style="color: orangered; font-size: large;">출발일 : </span>
                        <c:if test="${order.delivery.departureDate == null}">
                            미정
                        </c:if>
                        <c:if test="${order.delivery.departureDate != null}">
                            ${order.delivery.departureDate}
                        </c:if>
                        <br>
                        <span style="color: cornflowerblue; font-size: large">도착일 : </span>
                        <c:if test="${order.delivery.arrivalDate == null}">
                            미정
                        </c:if>
                        <c:if test="${order.delivery.arrivalDate != null}">
                            ${order.delivery.arrivalDate}
                        </c:if>
                    </p>
                    ...
                    <p class="card-text">
                        ...
                        <br>
                        <c:if test="${order.delivery.address.mainAddress == null}">
                            <span style="margin-left: 1em">미정</span>
                        </c:if>
                        <c:if test="${order.delivery.address.mainAddress != null}">
                            <span style="margin-left: 1em">${order.delivery.address.mainAddress}</span>
                        </c:if>
                        <br>
                        <span style="color: darkslateblue; font-size: large">상세 주소 : </span>
                        <br>
                        <c:if test="${order.delivery.address.detailAddress == null}">
                            <span style="margin-left: 1em">미정</span>
                        </c:if>
                        <c:if test="${order.delivery.address.detailAddress != null}">
                            <span style="margin-left: 1em">${order.delivery.address.detailAddress}</span>
                        </c:if>
                    </p>
                </div>
    ...
    $().ready(() => {
        ...
        $("#deliver-finish-btn").click((event) => {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '/seller/order/delivery/finish',
                        method: 'post',
                        type: 'json',
        ...
        $("#delivery-start-btn").click((event) => {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '/seller/order/delivery/start',
                        method: 'post',
                        type: 'json',
                        data: {deliveryId: deliveryId},
                        dataType: '',
